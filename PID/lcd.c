...
void disp(unsigned char x, unsigned char y,unsigned int speed)
{
	unsigned int tmp;
	tmp=speed;
	cursor_set(x,y);
  ...
  {
    lcd_write_dat('0'+tmp/1000%10);
	  lcd_write_dat('0'+tmp/100%10);
	  lcd_write_dat('0'+tmp/10%10); 
	  lcd_write_dat('0'+tmp%10);
  }
}
